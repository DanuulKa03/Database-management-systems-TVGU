-- Удаляем базу данных, если она уже существует (необязательно, но полезно для тестов)
DROP DATABASE IF EXISTS Сессия;
...
CREATE TABLE Студент (
                         ...
                         CHECK (N_группы ~ '^[1-6][A-Z]?\d{1,2}$') -- Проверка номера группы
    );
